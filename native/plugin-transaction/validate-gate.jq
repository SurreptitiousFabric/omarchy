...
def validate_v2:
  ...
  and (
    ...
    or (.state == "RELEASE_AUTHORIZED" and .unload == "acknowledged" and .rescan.outcome == "completed" and (.rescan.shellInstance | string and length > 0) and (.rescan.generation | type == "number" and . >= 0 and floor == .) and (.rescan.scanEpoch | type == "number" and . >= 0 and floor == .) and .release.outcome == "authorized" and (.release.shellInstance | string and length > 0) and (.release.generation | type == "number" and . >= 0 and floor == .) and (.release.configurationEpoch | type == "number" and . >= 0 and floor == .) and .release.shellInstance == .rescan.shellInstance and .release.generation == .rescan.generation and ((.expected.targetRole == "absence" and .rescan.expectedTree == null and .rescan.observedTree == null) or (.expected.targetRole != "absence" and .rescan.expectedTree == .expected.tree and .rescan.observedTree == .expected.tree)) and .terminalReceipt.state == "not-requested")
    or (.state == "TERMINAL_RECEIPT" and .terminalReceipt.state == "durable" and ((.terminalReceipt.intendedJournalState == "COMMITTED" and .expected.targetRole == "candidate") or (.terminalReceipt.intendedJournalState == "ROLLED_BACK" and (.expected.targetRole == "prior-tree" or .expected.targetRole == "absence"))) and .terminalReceipt.operationBindingSha256 == .operationBindingSha256 and .terminalReceipt.operationId == .operationId and .terminalReceipt.pluginId == .pluginId and .terminalReceipt.targetRole == .expected.targetRole and ((.expected.targetRole == "absence" and .terminalReceipt.target == {state:"absent",identity:null}) or (.expected.targetRole != "absence" and .terminalReceipt.target.state == "present" and .terminalReceipt.target.identity == .expected.tree)) and (.terminalReceipt.shellInstance | string and length > 0) and (.terminalReceipt.generation | type == "number" and . >= 0 and floor == .) and (.terminalReceipt.scanEpoch | type == "number" and . >= 0 and floor == .) and (.terminalReceipt.configurationEpoch | type == "number" and . >= 0 and floor == .))
  );
