...
3. `releasePlugin(operationId, pluginId, expectedTree, generation)` removes only the matching gate, recomputes eligibility against current configuration, and acknowledges publication of the resulting registry state. It does not claim that on-demand or asynchronous code executed successfully.
...
- From acknowledged gate through release or exact rollback, every loader treats the plugin as ineligible.
